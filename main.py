...
def solve_FJSP(file_num,run_times):
    ...
    fitness_list = [Decode.decode(chr, job_op_num, p_table, 'decode',None)[0] for chr in P]
    ...
    for iter in range(Iter):
        ...
        chrs = [pso.f_operator(job_op_num, p_table, chr, P[index], Pg, pf, o_mega, c1, c2) for index, chr in
                enumerate(copy_chrs)]
        # 更新个体最优位置
        P = np.array([chr1 if Decode.decode(chr1, job_op_num, p_table, 'decode',None)[0] <= Decode.decode(chr2, job_op_num,
                                                                                                  p_table, 'decode',None)[0]
        ...
        top3 = np.sort(np.unique(fitness_list))[:3]
        # 这里找到所有全局最优解，然后局部搜索一下
        #目前是找的排名前三的所有染色体
        Pgs_index=[]
        for i in top3:
            Pgs_index.extend(list(np.where(fitness_list == i))[0])

        # print(Pgs_index)


        for index in Pgs_index:
            # print(Decode.decode(P[index], job_op_num, p_table, 'decode',None)[0])
            #局部搜索后重新覆盖前三的染色体
            P[index] = LocalSearch.LS(P[index],job_op_num,p_table)
        ...
        print("第"+str(file_num+1)+'个数据集，第'+str(run_times+1)+'/3次运行'+'迭代：'+str(iter+1)+'/'+str(Iter))

    fitness = Decode.decode(Pg, job_op_num, p_table, 'decode',None)[0]
